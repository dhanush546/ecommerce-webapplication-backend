...
userApp.put('/add-to-cart/:username',expressAsyncHandler(async(req,res)=>{
    //get cartCollection obj
    let usersCollection=req.app.get('usersCollection')
    //get username from url
    let usernameOfUrl=req.params;
    // console.log(usernameOfUrl)
    //get product obj
    let productObj=req.body;
    // console.log(productObj);
    let result=await usersCollection.updateOne({username:usernameOfUrl},{$push:{products:productObj}})
...
userApp.put('/delete-from-cart/:username',expressAsyncHandler(async(req,res)=>{
  ...
  let productObj=(req.body);
  // console.log(productObj)
  await usersCollection.updateOne({username:usernameFromUrl},{$pull:{products:{id:productObj}}})
...
userApp.get('/cart/:username',expressAsyncHandler(async(req,res)=>{
  ...
  let usernameFromUrl=req.params.username;
  //get cart
  let user=await usersCollection.findOne({username:{$eq:usernameFromUrl}})
  //send res
